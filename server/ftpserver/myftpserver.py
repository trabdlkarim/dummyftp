...
class MyFTPServer(ThreadingMixIn, UDPServer):
    ...
    def init_setup(self, host=HOST, port=PORT):
         ...
         self.data_socket.bind((host,port))
         time.sleep(2)
    ...
    def send_response(self,retcode, message):
        ...
        self.data_socket.sendto(bytes(str(retcode) + message ,"utf-8"),self.client_address)
        print("Return code sent.")
     
    def handle_client_request(self,host=HOST, port=PORT):
        
         self.client_request, self.client_address = self.data_socket.recvfrom(1024)
            ...
            if command == "RETR":
                ...
                if type(dir_contents) == tuple:
                    ...
                    with open(self.server.filename,'wb') as file:
                        while fdata:    
                            file.write(fdata)
                            fdata = self.rfile.readline(4096)
                        self.transfer_data(bytes(str("File has been successfully uploaded  server uploads directory."),"utf-8"))
